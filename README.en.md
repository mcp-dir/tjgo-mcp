...
**Manual** (if the deeplink does not open): [claude.ai/customize/connectors](https://claude.ai/customize/connectors?surface=cowork) → **+** → **Add custom connector** → paste **Name** `Jurisprudência TJGO` and **URL** `https://api.mcp.ai/p_tjgo`.

### Cursor

[➕ Install Jurisprudência TJGO in Cursor](cursor://anysphere.cursor-deeplink/mcp/install?name=tjgo&config=eyJ1cmwiOiJodHRwczovL2FwaS5tY3AuYWkvcF90amdvIn0=)

### VS Code (Copilot Chat)

[➕ Install Jurisprudência TJGO in VS Code](vscode:mcp/install?name=tjgo&config=%7B%22type%22%3A%22http%22%2C%22url%22%3A%22https%3A%2F%2Fapi.mcp.ai%2Fp_tjgo%22%7D)
...
```
Recent TJGO rulings on consumer damages
Compare TJGO and the Superior Court on limitation periods
...
**O TJGO é o único tribunal?**
Não. A conexão serve 17 tribunais: além do TJGO, STF, STJ, TST, TRF3, TRF4, CARF e os demais tribunais de justiça estaduais. Restringir a busca ao TJGO é opcional, e comparar com as cortes superiores é uma pergunta só.

**O acervo do TJGO está atualizado?**
O acervo publicado deste tribunal está com defasagem. A própria resposta avisa isso, com a data mais recente que encontrou, para você conferir no site oficial antes de tratar uma decisão como o estado atual da jurisprudência. Os outros 16 tribunais da mesma conexão não são afetados.
...
- 📧 [tjgo@mcp.ai](mailto:tjgo@mcp.ai)
- 🐛 [GitHub Issues](https://github.com/mcp-dir/tjgo-mcp/issues)
- 📄 [docs/](docs/)
...
MIT — see [LICENSE](LICENSE). The MCP server at `api.mcp.ai/p_tjgo` is proprietary (hosted); this repo (manifests, docs, skills) is MIT.
