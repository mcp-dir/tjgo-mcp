---
name: tjgo-mcp
description: Pesquisa de jurisprudência do Tribunal de Justiça de Goiás (TJGO) e de mais 16 tribunais brasileiros: acórdãos, decisões e súmulas por termo ou tese, com o trecho que casou a busca, o link oficial e leitura do inteiro teor quando disponível. Use sempre que o usuário pedir precedente, jurisprudência, súmula ou entendimento de tribunal. Restrinja com tribunais:["TJGO"] quando ele indicar TJGO, e compare com as cortes superiores quando a pergunta pedir. Zero resultado costuma ser vocabulário, não ausência: tente os termos alternativos que a resposta sugere antes de afirmar que a tese não existe, e nunca trate aviso de indisponibilidade como inexistência. Grátis, sem login. Orquestra jurisprudencia_buscar, jurisprudencia_sumulas e jurisprudencia_documento em https://api.mcp.ai/p_tjgo.
---

# Jurisprudência TJGO — REST API skill

Você tem acesso à **Jurisprudência TJGO** REST API na MCP.AI.
...
Este MCP também funciona via **conexão MCP** (Claude / Cursor) em `https://api.mcp.ai/p_tjgo` — veja o [README](../../README.md). A skill acima é pra consumir a **REST API** direto (agente próprio / código).
